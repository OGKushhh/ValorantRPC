...
class Presence:
    ...
    def _connect_rpc(self, startup=False):
        ...
        if self.cached_account_level is not None and self.cached_account_level > 0:
            return self.cached_account_level
        
        # Sinon, récupérer le niveau
        level = Utilities.fetch_account_level(self.client, data)
        
        # Mettre en cache si valide
        if level and level > 0:
            self.cached_account_level = level
        ...
        while True:
            ...
            if presence_data is not None:
                ...
                if not session_state or (isinstance(session_state, str) and not session_state.strip()):
                    ...
                    # First check if we're in pregame (agent selection)
                    in_pregame = False
                    try:
                        from valclient.exceptions import PhaseError
                        pregame = self.client.pregame_fetch_player()
                        if pregame is not None:
                            in_pregame = True
                            self.update_presence("PREGAME", presence_data)
                    except PhaseError:
                        in_pregame = False
                    except Exception as e:
                        in_pregame = False
                    ...
                    if not in_pregame:
                        # Not in pregame, check if we're in game
                        in_game = False
                        try:
                            from valclient.exceptions import PhaseError
                    ...
                    # If sessionLoopState is present, use it but also verify pregame state
                    # This ensures we catch pregame even if sessionLoopState is set incorrectly
                    if session_state.upper() != "PREGAME":
                        try:
                            from valclient.exceptions import PhaseError
                            pregame = self.client.pregame_fetch_player()
                            if pregame is not None:
                                # Override session_state if we're actually in pregame
                                self.update_presence("PREGAME", presence_data)
                            else:
    ...
    def init_loop(self):
        try:
            ...
            if presence_data is not None:
                ...
                # Always try to update presence if we have data
                if not session_state or (isinstance(session_state, str) and not session_state.strip()):
                    # Check if we're in pregame first (agent selection)
                    in_pregame = False
                    try:
                        from valclient.exceptions import PhaseError
                        pregame = self.client.pregame_fetch_player()

...
class Startup:
    def __init__(self):
        if not Processes.is_program_already_running():
            ...
            # ── share config with GUI ─────────────────────────────────────────
            GUIState.config = self.config
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                f"valorant-rpc {Localizer.get_config_value('version')}")

            color_print([(
                "Red", Localizer.get_localized_text("prints", "startup", "wait_for_rpc"))])
            try:
                ...
                traceback.print_exc()
                color_print([("Cyan",
                    f"{Localizer.get_localized_text('prints','startup','discord_not_detected')} ({e})")])
                if not Processes.are_processes_running():
                    color_print([("Red",
                        Localizer.get_localized_text("prints", "startup", "starting_valorant"))])
                    self.start_game()
                    os._exit(1)
    ...
    def run(self):
        ...
        # ── share presence object with GUI ────────────────────────────────────
        GUIState.presence_ref = self.presence
        ...
        color_print([("LimeGreen",
            f"{Localizer.get_localized_text('prints','startup','startup_successful')}\n")])

        # The systray thread keeps the process alive.
        # We do NOT call join() here — the GUI window and systray together
        # keep everything running.  We just park this thread.
    ...
    def dispatch_systray(self):
        # Non-daemon so the process stays alive as long as the systray is up.
        self.systray_thread = Thread(target=self.systray.run)
        self.systray_thread.start()

    # ── valorant client setup ─────────────────────────────────────────────────

    def setup_client(self):
        max_attempts = 10
        for attempt in range(max_attempts):
            try:
                self.client = valclient.Client(
                ...
                if attempt == 0:
                    print()
                Startup.clear_line()
                color_print([("Cyan", "["), ("White", f"{attempt+1}/{max_attempts}"),
                             ("Cyan", "] Waiting for Valorant client...")])
                time.sleep(3)
    ...
    def wait_for_presence(self):
        ...
        while self.client.fetch_presence() is None:
            Startup.clear_line()
            color_print([("Cyan", "["), ("White", f"{presence_timer}"),
                         ("Cyan", f"] {Localizer.get_localized_text('prints','startup','waiting_for_presence')}")])
            presence_timer += 1
    ...
    def start_game(self):
        ...
        psutil.subprocess.Popen(
            [path, "--launch-product=valorant", "--launch-patchline=live"])
        print()
        while not Processes.are_processes_running():
            Startup.clear_line()
            color_print([("Cyan", "["), ("White", f"{launch_timer}"),
                         ("Cyan", f"] {Localizer.get_localized_text('prints','startup','waiting_for_valorant')}")])
            launch_timer += 1
    ...
    def check_region(self):
        color_print([("Red bold",
            Localizer.get_localized_text("prints", "startup", "autodetect_region"))])
            ...
            if session["productId"] == "valorant":
                ...
                for arg in launch_args:
                    if "-ares-deployment" in arg:
                        ...
                        color_print([("LimeGreen",
                            f"{Localizer.get_localized_text('prints','startup','autodetected_region')} "
                            f"{Localizer.get_config_value('region', 0)}")])
                        time.sleep(5)
                        Systray.restart()
